...
def get_thick_slices(img_data, slice_thickness=3):
    ...
    for slice_idx in range(2 * slice_thickness + 1):
        img_data_thick = np.append(
            img_data_thick, img_data_pad[:, :, slice_idx : d + slice_idx, :], axis=3
        )
...
def fill_unknown_labels_per_hemi(gt, unknown_label, cortex_stop):
    ...
    for idx in range(len(list_parcels)):
        aseg_blur = filters.gaussian_filter(
            1000 * np.asarray(gt == list_parcels[idx], dtype=np.float32), sigma=5
        )
...
class OrigDataThickSlices(Dataset):
    ...
    def __init__(
        self, img_filename, orig, plane="Axial", slice_thickness=3, transforms=None
    ):
...
class AsegDatasetWithAugmentation(Dataset):
    ...
    def __init__(self, params, transforms=None):
        ...
        try:
            ...
            print(
                "Successfully loaded {} with plane: {}".format(
                    params["dataset_name"], params["plane"]
                )
            )
    ...
    def __init__(self, params, transforms=None):
        ...
        try:
            ...
            self.hdf5_files = sorted(
                glob.glob(os.path.join(params["dataset_name"], "*hdf5"))
            )
